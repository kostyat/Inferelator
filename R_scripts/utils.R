...
read.input <- function(input.dir, exp.mat.file, tf.names.file, meta.data.file, 
           priors.file, gold.standard.file, leave.out.file,  deg.rates.file) {
  IN <- list()
  ...
  IN$original.exp.mat.rns = rownames(IN$exp.mat)
  ...
  if (!is.null(gold.standard.file)) {
    IN$gs.mat <- abs(as.matrix(read.table(file=file.path(input.dir, gold.standard.file),
                                    row.names=1, header=T, sep='\t', check.names=F)))
  }
  ...
  if(!is.null(deg.rates.file)) {
    IN$deg.mat <- NULL
    IN$deg.mat <-  as.matrix(read.table(file=file.path(input.dir, deg.rates.file),
                                    row.names=1, header=T, sep='\t', check.names=F))
  }
